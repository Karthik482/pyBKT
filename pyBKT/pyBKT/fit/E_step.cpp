...
//
//  Created by CristiÃƒÂ¡n Garay on 11/16/16.
...
dict run(dict& data, dict& model, np::ndarray& trans_softcounts, np::ndarray& emission_softcounts, np::ndarray& init_softcounts, int num_outputs){
    ...
    //printf("E STEP CALLED");
    np::ndarray alldata = extract<np::ndarray>(data["data"]); //multidimensional array, so i need to keep extracting arrays.
    ...
    np::ndarray allresources = extract<np::ndarray>(data["resources"]);

    np::ndarray starts = extract<np::ndarray>(data["starts"]);

    int num_sequences = len(starts);

    np::ndarray lengths = extract<np::ndarray>(data["lengths"]);

    np::ndarray learns = extract<np::ndarray>(model["learns"]);
    int num_resources = len(learns);

    np::ndarray forgets = extract<np::ndarray>(model["forgets"]);

    np::ndarray guesses = extract<np::ndarray>(model["guesses"]);
    //int num_subparts = len(guesses);
    //printf("12345678");
    np::ndarray slips = extract<np::ndarray>(model["slips"]);
    ...
    #pragma omp parallel
    {
        ...
        for (int sequence_index=sequence_idx_start; sequence_index < sequence_idx_end; sequence_index++) {
            ...
            long int sequence_start = extract<long int>(starts[sequence_index]) - 1;
            ...
            likelihoods.setOnes();
             for (int t=0; t<T; t++) {
                 for (int n=0; n<num_subparts; n++) {
                    int data_temp = extract<int>(alldata[n][sequence_start+t]);
                     if (data_temp != 0) {
            ...
            for (int t=0; t<T-1; t++) {
                long int resources_temp = extract<long int>(allresources[sequence_start+t]);
            ...
            for (int n=0; n<num_subparts; n++) {
                int data_temp = extract<int>(alldata[n][sequence_start+(T-1)]);
                if (data_temp != 0) {
    ...
    boost::python::numpy::ndarray all_trans_softcounts_arr( all_trans_softcounts_handle );
    result["all_trans_softcounts"] = all_trans_softcounts_arr;*/
    
    /*
        boost::python::tuple orig_shape_trans_oned = boost::python::make_tuple(num_resources, 2, 2);
    boost::python::tuple orig_shape_trans = boost::python::make_tuple(num_resources, 2, 2);
    np::ndarray all_trans_softcounts_arr = np::zeros(orig_shape_trans, np::dtype::get_builtin<double>());
    std::copy(&r_trans_softcounts[0], &r_trans_softcounts[num_resources*2*2 - 1], reinterpret_cast<double*>(all_trans_softcounts_arr.get_data()));
    */
    
    /*
    boost::python::list l_trans;
    for(int i=0;i<num_resources*2*2;i++){
        l_trans.append(r_trans_softcounts[i]);
    }
    np::ndarray all_trans_softcounts_arr = np::array(l_trans);
    boost::python::tuple orig_shape_trans = boost::python::make_tuple(num_resources, 2, 2);
    result["all_trans_softcounts"] = all_trans_softcounts_arr.reshape(orig_shape_trans);
    */
    ...
    boost::python::numpy::ndarray all_emission_softcounts_arr( all_emission_softcounts_handle );
    result["all_emission_softcounts"] = all_emission_softcounts_arr;*/
    
    /*
    boost::python::list l_em;
    for(int i=0;i<num_subparts*2*2;i++){
        l_em.append(r_emission_softcounts[i]);
    }
    np::ndarray all_emission_softcounts_arr = np::array(l_em);
    boost::python::tuple orig_shape_em = boost::python::make_tuple(num_subparts, 2, 2);
    result["all_emission_softcounts"] = all_emission_softcounts_arr.reshape(orig_shape_em);
    */
    ...
    boost::python::numpy::ndarray all_initial_softcounts_arr( all_initial_softcounts_handle );
    result["all_initial_softcounts"] = all_initial_softcounts_arr;
    */
    
    /*
    boost::python::list l_init;
    for(int i=0;i<2;i++){
        l_init.append(r_init_softcounts[i]);
    }
    np::ndarray all_initial_softcounts_arr = np::array(l_init);
    boost::python::tuple orig_shape_init = boost::python::make_tuple(2, 1);
    result["all_initial_softcounts"] = all_initial_softcounts_arr.reshape(orig_shape_init);
    */
    ...
    boost::python::numpy::ndarray alpha_out_arr( alpha_out_handle );
    result["alpha"] = alpha_out_arr;*/
    
    /*
    boost::python::list l_alpha;
    for(int i=0;i<2*bigT;i++){
        l_alpha.append(r_alpha_out[i]);
    }
    np::ndarray alpha_out_arr = np::array(l_alpha);
    boost::python::tuple orig_shape_alpha = boost::python::make_tuple(2, bigT);
    result["alpha"] = alpha_out_arr.reshape(orig_shape_alpha);*/
    
    boost::python::tuple orig_shape_trans = boost::python::make_tuple(num_resources, 2, 2);
    np::ndarray final_trans_arr = np::zeros(orig_shape_trans, np::dtype::get_builtin<int>());
    for(int i=0;i<num_resources;i++){
        for(int j=0;j<2;j++){
            for(int k=0;k<2;k++){
                final_trans_arr[i][j][k] = r_trans_softcounts[i*4 + j*2 + k];
            }
        }
    }
    result["all_trans_softcounts"] = final_trans_arr;
    
    boost::python::tuple orig_shape_em = boost::python::make_tuple(num_subparts, 2, 2);
    np::ndarray final_em_arr = np::zeros(orig_shape_em, np::dtype::get_builtin<int>());
    for(int i=0;i<num_subparts;i++){
        for(int j=0;j<2;j++){
            for(int k=0;k<2;k++){
                final_em_arr[i][j][k] = r_emission_softcounts[i*4 + j*2 + k];
            }
        }
    }
    result["all_emission_softcounts"] = final_em_arr;
    
    boost::python::tuple orig_shape_init = boost::python::make_tuple(2, 1);
    np::ndarray final_init_arr = np::zeros(orig_shape_init, np::dtype::get_builtin<int>());
    for(int i=0;i<2;i++){
        for(int j=0;j<1;j++){
                final_init_arr[i][j] = r_init_softcounts[i];
        }
    }
    result["all_initial_softcounts"] = final_init_arr;
    
    
    boost::python::tuple orig_shape_alpha = boost::python::make_tuple(2, bigT);
    np::ndarray final_alpha_arr = np::zeros(orig_shape_alpha, np::dtype::get_builtin<int>());
    for(int i=0;i<2;i++){
        for(int j=0;j<bigT;j++){
            final_alpha_arr[i][j] = r_alpha_out[i];
        }
    }
    result["alpha"] = final_alpha_arr;
    return(result);
}
