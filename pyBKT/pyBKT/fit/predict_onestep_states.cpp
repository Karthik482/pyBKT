...
numpy::ndarray run(dict& data, dict& model, numpy::ndarray& forward_messages){
    ...
    boost::python::numpy::ndarray all_predictions_arr( all_predictions_handle );*/
    
    /*boost::python::list l_pred;
    for(int i=0;i<2*bigT;i++){
        l_pred.append(all_predictions[i]);
    }
    np::ndarray all_predictions_arr = np::array(l_pred);
    boost::python::tuple orig_shape_pred = boost::python::make_tuple(2, bigT);
    np::ndarray all_predictions_final = all_predictions_arr.reshape(orig_shape_pred)*/
    boost::python::tuple orig_shape = boost::python::make_tuple(2, bigT);
    np::ndarray final_predictions_arr = np::zeros(orig_shape, np::dtype::get_builtin<double>());
    for(int i=0;i<2;i++){
        for(int j=0;j<bigT;j++){
            final_predictions_arr[i][j] = all_predictions[i*bigT + j];
        }
    }
    return final_predictions_arr;
}
